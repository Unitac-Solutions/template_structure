...
const Patient_case = require("../services/patient_case.service")

const getpatient_cases = asyncHandler( async ( req, res) => {
    const patient_cases = await Patient_case.getpatient_cases()
...
const createpatient_case = asyncHandler(  async ( req, res)=> {
    ...
    if (!open_case) {
        ...
        await  Patient_case.createpatient_case(req.body);
        res.status(201).send( "Created Succesfully.")
});

const getpatient_case = asyncHandler( async ( req, res) => {
    const patient_case = await Patient_case.getpatient_case(req.params.id);
    if(!patient_case ){
        res.status(404);
...
const updatepatient_case = asyncHandler(  async ( req, res)=> {
    const patient_case = await Patient_case.updatepatient_case(req.body, req.params.id);
    if(!patient_case){
        res.status(404);
        throw new Error("patient_case not found");
    }
...
const deletepatient_case = asyncHandler( async ( req, res)=> {
    const record =  await  Patient_case.deletepatient_case(req.params.id);
    if (!record)
