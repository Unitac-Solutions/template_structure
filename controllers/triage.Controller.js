...
const createTriage = asyncHandler(  async ( req, res)=> {
    const {tew_id,user_type, branch, username, email, user_reg, street_name, city_town, province, postal_code} = req.body;
    if (!tew_id || !user_type || !branch || !username || !email || !user_reg || !street_name || !city_town || !province || !postal_code) {
